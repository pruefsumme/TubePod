...
NSString * const TPDatabaseErrorDomain = @"com.pruefsumme.tubepod.database";
static NSString * const TPDatabasePath = @"/var/mobile/Media/iTunes_Control/iTunes/MediaLibrary.sqlitedb";

static NSError *TPDatabaseError(sqlite3 *database, NSInteger code, NSString *fallback) {
    const char *message = database ? sqlite3_errmsg(database) : NULL;
    return [NSError errorWithDomain:TPDatabaseErrorDomain code:code userInfo:@{NSLocalizedDescriptionKey: message && *message ? [NSString stringWithUTF8String:message] : (fallback ?: @"Music database query failed.")}];
}
...
static NSSet *TPQueryIDs(NSString *sql, NSArray *values, NSError **error) {
    ...
    int status = sqlite3_open_v2(TPDatabasePath.UTF8String, &database, SQLITE_OPEN_READONLY, NULL);
    if (status != SQLITE_OK) {
        if (error) *error = TPDatabaseError(database, status, @"Music could not open its read-only library database.");
    ...
    if (status != SQLITE_OK) {
        if (error) *error = TPDatabaseError(database, status, @"Music could not prepare its read-only library query.");
    ...
    for (NSUInteger index = 0; index < values.count; index++) {
        ...
        if (status != SQLITE_OK) {
            if (error) *error = TPDatabaseError(database, status, @"Music could not bind its read-only library query.");
    ...
    if (status != SQLITE_DONE) {
        if (error) *error = TPDatabaseError(database, status, @"Music could not read its library query result.");
        result = nil;
    }
...
+ (NSSet *)allTrackIDsForTitle:(NSString *)title album:(NSString *)album error:(NSError **)error {
    if (!title.length || !album.length) { if (error) *error = [NSError errorWithDomain:TPDatabaseErrorDomain code:1 userInfo:@{NSLocalizedDescriptionKey: @"Music received empty track matching fields."}]; return nil; }
...
+ (NSSet *)emptyPlaceholderTrackIDsForTitle:(NSString *)title album:(NSString *)album error:(NSError **)error {
    if (!title.length || !album.length) { if (error) *error = [NSError errorWithDomain:TPDatabaseErrorDomain code:1 userInfo:@{NSLocalizedDescriptionKey: @"Music received empty placeholder matching fields."}]; return nil; }
...
+ (NSSet *)allEmptyPlaceholderTrackIDsForAlbum:(NSString *)album error:(NSError **)error {
    if (!album.length) { if (error) *error = [NSError errorWithDomain:TPDatabaseErrorDomain code:1 userInfo:@{NSLocalizedDescriptionKey: @"Music received an empty album matching field."}]; return nil; }
...
+ (NSDictionary *)recordForPersistentID:(NSNumber *)persistentID error:(NSError **)error {
    sqlite3 *database = NULL;
    sqlite3_stmt *statement = NULL;
    if (!persistentID) { if (error) *error = [NSError errorWithDomain:TPDatabaseErrorDomain code:1 userInfo:@{NSLocalizedDescriptionKey: @"Music received an empty persistent ID."}]; return nil; }
    int status = sqlite3_open_v2(TPDatabasePath.UTF8String, &database, SQLITE_OPEN_READONLY, NULL);
    if (status != SQLITE_OK) { if (error) *error = TPDatabaseError(database, status, @"Music could not open its read-only library database."); if (database) sqlite3_close(database); return nil; }
    sqlite3_busy_timeout(database, 1000);
    ...
    if (status != SQLITE_OK || bindStatus != SQLITE_OK) {
        ...
        if (error) *error = TPDatabaseError(database, status, @"Music could not prepare its persistent-ID query.");
    ...
    if (status != SQLITE_ROW) { if (error) *error = TPDatabaseError(database, status, @"Music could not read its persistent-ID query."); sqlite3_finalize(statement); sqlite3_close(database); return nil; }
